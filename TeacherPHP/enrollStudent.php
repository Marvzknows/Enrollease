<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST')
    {
        ...
        $lrn =  $_POST['lrn'];
        // For aactivity logs
        $description = 'Successfuly Enrolled Student: '.$lrn;
            ...
            if($secLimit = mysqli_num_rows($capacityQuery) >= 65) // CHANGE TO 60 SOON
            {
                ...
                // Inserting Student
                    ...
                    if ($enrollQuery) {
                        ...
                        $tchRegularLogs_sql = "INSERT INTO activity_log (teacher_id, description, date_log, time)
                        VALUES ('$teacherId', '$description', '$currentDate', '$time')";
                        $tchRegularLogs_Query = mysqli_query($connection,$tchRegularLogs_sql);

                        if($tchRegularLogs_Query) {
                            $output = [
                                'status' => 'success',
                                'message' => 'Enrolled'
                            ];
        ...
        // Regular Statement End
            ...
            if($checkLrnForIrreg = mysqli_num_rows($checkIrregLrnQuery) > 0)
            {
                ...
                echo json_encode($output);
                    ...
                    if ($irregEnrollQuery) {
                        // Teacher Activity Log
                        $tchTransfereeLogs_sql = "INSERT INTO activity_log (teacher_id, description, date_log, time)
                        VALUES ('$teacherId', '$description', '$currentDate', '$time')";
                        $tchTransfereeLogs_Query = mysqli_query($connection,$tchTransfereeLogs_sql);

                        if($tchTransfereeLogs_Query) {
                            $output = [
                                'status' => 'success',
                                'message' => 'Enrolled'
                            ];
