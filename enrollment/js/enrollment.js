$(document).ready(function () {
  ...
  function insertRegular(schoolyear,lrn,gradeLevel,section,studentType,studentFname,studentMname,studentlname,
    ...
    $.ajax({
      type: "post",
      url: "enrollment/enrollStudent.php",
      data: {
        'schoolyear':schoolyear,
        'lrn':lrn,
        'gradeLevel':gradeLevel,
...
// NAHINTO SA VALIDATION NG STUDENT TYPE
  ...
  // Display error for number input type
  $('#lrn').on('keypress', function (event) {
    ...
    if (keyCode >= 48 && keyCode <= 57) {
        $('#lrnError').text('');
        // alert('Key pressed is a number.');
    }
    // Check if the key pressed is a letter (a-z or A-Z)
    else if ((keyCode >= 65 && keyCode <= 90) || (keyCode >= 97 && keyCode <= 122)) {
        $('#lrnError').text('Invalid input type, Enter number type only');
        // alert('Key pressed is a letter.');
    }
    // If the key pressed is neither a number nor a letter
    // else {
    //     alert('Key pressed is neither a number nor a letter.');
    // }
});
